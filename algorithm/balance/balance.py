...
def swap_users_for_geographic_separation(routes):
    ...
    1. Traditional user swaps (exchange users between routes)
    2. Asymmetric moves (move users to routes with available capacity)
    ...
    # Find nearby route pairs (similar to existing logic but more targeted)
    proximity_threshold = 4.0  # km - increased to catch more overlapping cases
    ...
    for i, route1 in enumerate(routes_copy):
        ...
        for j, route2 in enumerate(routes_copy):
            ...
            if distance <= proximity_threshold:
                ...
                # Perform best asymmetric moves
                for move in asymmetric_moves[:1]:  # Only perform the best move per route pair
                    logger.info(f"      🚀 Attempting asymmetric move: {move['user_id']} ({move['direction']})")
                    ...
                    if perform_asymmetric_user_move(move['source_route'], move['target_route'], move):
                        moves_made += 1
                        logger.info(f"      ✅ Moved user {move['user_id']} from route {move['source_route']['driver_id']} to route {move['target_route']['driver_id']}")
                        break  # Only do one move per route pair to avoid over-optimization
                    else:
                        logger.warning(f"      ❌ Failed to move user {move['user_id']} from route {move['source_route']['driver_id']} to route {move['target_route']['driver_id']}")

                # Step 2: If no good asymmetric moves, try traditional swaps
                if not asymmetric_moves or moves_made == 0:
                    ...
                    # Perform best swaps
                    for swap in swap_candidates[:1]:  # Limit to one swap per route pair
                        logger.info(f"      🔄 Attempting swap: {swap['user1_id']} (route {route1['driver_id']}) ↔ {swap['user2_id']} (route {route2['driver_id']})")
                        ...
                        if perform_user_swap(route1, route2, swap):
                            ...
                            break  # Only do one swap per route pair
                        else:
    ...
    if total_optimizations > 0:
        logger.info(f"   ✅ User optimization complete: {moves_made} asymmetric moves, {swaps_made} swaps made for better geographic separation")
    else:
...
def find_user_swap_candidates(route1, route2, center1, center2):
    ...
    Focus on creating cleaner geographic boundaries even if individual users travel slightly further
    """
    swap_candidates = []
    ...
    # Calculate distances for each user to both route centers
    ...
    for user in route1['assigned_users']:
        ...
        prefers_other = dist_to_center2 < dist_to_center1 - 0.1  # Very low tolerance - more aggressive
        ...
        if prefers_other:
            logger.debug(f"         User {user['user_id']} from route {route1['driver_id']} prefers route {route2['driver_id']} (improvement: {improvement:.3f}km)")
    ...
    for user in route2['assigned_users']:
        ...
        dist_to_center2 = haversine_distance(user_pos[0], user_pos[1], center2[0], center2[1])
        dist_to_center1 = haversine_distance(user_pos[0], user_pos[1], center1[0], center1[1])
        prefers_other = dist_to_center1 < dist_to_center2 - 0.1  # Very low tolerance - more aggressive
        ...
        if prefers_other:
            logger.debug(f"         User {user['user_id']} from route {route2['driver_id']} prefers route {route1['driver_id']} (improvement: {improvement:.3f}km)")

    # Strategy 1: Find mutually beneficial swaps (both users prefer the other route)
    mutual_swaps_found = 0
    for user1_data in route1_user_analysis:
        if not user1_data['prefers_other_route']:
            continue

        for user2_data in route2_user_analysis:
            if not user2_data['prefers_other_route']:
                continue

            total_improvement = user1_data['improvement'] + user2_data['improvement']

            logger.debug(f"         Mutual swap: {user1_data['user_id']} ↔ {user2_data['user_id']}, total improvement: {total_improvement:.3f}km")

            if total_improvement > 0.1:  # Very low threshold for mutual swaps
                ...
                mutual_swaps_found += 1
                logger.info(f"         ✅ Mutual swap: {user1_data['user_id']} ↔ {user2_data['user_id']}, improvement: {total_improvement:.3f}km")

    # Strategy 2: Aggressive geographic proximity swaps
    # Prioritize keeping nearby users together even more strongly
    if mutual_swaps_found == 0:
        logger.debug(f"      No mutual swaps found, looking for aggressive geographic proximity swaps...")

        # Create a list of all potential swaps with their geographic proximity scores
        all_potential_swaps = []

        for user1_data in route1_user_analysis:
            for user2_data in route2_user_analysis:
                # Calculate geographic proximity score for both users
                user1_to_route2_score = user1_data['improvement']  # Positive if closer to route2
                user2_to_route1_score = user2_data['improvement']  # Positive if closer to route1

                # Geographic proximity benefit: how much closer these users are to each other's route centers
                proximity_benefit = user1_to_route2_score + user2_to_route1_score

                # Also calculate how close these users are to each other (bonus for very close users)
                ...
                # Bonus if users are very close to each other (they should be in same route)
                proximity_bonus = max(0, (1.0 - user_distance) * 2.0)  # Strong bonus for close users

                # Total score combines proximity benefit and user closeness
                total_score = proximity_benefit + proximity_bonus

                all_potential_swaps.append({
                    'user1_id': user1_data['user_id'],
                    'user1_data': user1_data['user'],
                    'user2_id': user2_data['user_id'],
                    'user2_data': user2_data['user'],
                    'improvement_score': total_score,
                    'proximity_benefit': proximity_benefit,
                    'user_distance': user_distance,
                    'route1_capacity_after': len(route1['assigned_users']),
                    'route2_capacity_after': len(route2['assigned_users']),
                    'swap_type': 'aggressive_geographic'
                })

                logger.debug(f"         Potential swap: {user1_data['user_id']} ↔ {user2_data['user_id']}")
                logger.debug(f"            Proximity benefit: {proximity_benefit:.3f}km, User distance: {user_distance:.3f}km, Total score: {total_score:.3f}")

        # Sort by total score (highest first) and add the best swaps
        all_potential_swaps.sort(key=lambda x: x['improvement_score'], reverse=True)

        # Find the best swap with special priority for geographic proximity
        best_swap = None

        # Priority 1: Look for swaps with very close users (< 0.8km) AND reasonable proximity benefit
        for swap in all_potential_swaps:
            if swap['user_distance'] < 0.8 and swap['proximity_benefit'] > -0.2:
                best_swap = swap
                logger.info(f"         🎯 PRIORITY 1: Very close users with reasonable benefit!")
                break

        # Priority 2: Look for swaps where both users strongly prefer the other route
        if not best_swap:
            for swap in all_potential_swaps:
                user1_prefers = swap['proximity_benefit'] > 0.3  # Strong preference
                user2_prefers = swap['proximity_benefit'] > 0.3  # Both benefit
                close_users = swap['user_distance'] < 1.0  # Users are close to each other

                if user1_prefers and user2_prefers and close_users:
                    best_swap = swap
                    logger.info(f"         🎯 PRIORITY 2: Both users strongly prefer swap and are close!")
                    break

        # Priority 3: Look for swaps with positive proximity benefit and close users
        if not best_swap:
            for swap in all_potential_swaps:
                if (swap['proximity_benefit'] > 0 and swap['user_distance'] < 1.2):
                    best_swap = swap
                    logger.info(f"         🎯 PRIORITY 3: Positive benefit + close users!")
                    break

        # Priority 4: Look for any swaps with positive proximity benefit
        if not best_swap:
            for swap in all_potential_swaps:
                if (swap['proximity_benefit'] > 0 and swap['user_distance'] < 2.5):
                    best_swap = swap
                    logger.info(f"         🎯 PRIORITY 4: Positive proximity benefit found!")
                    break

        # Priority 5: Any reasonable swap
        if not best_swap:
            for swap in all_potential_swaps:
                if (swap['proximity_benefit'] > -0.8 or swap['user_distance'] < 0.5):
                    best_swap = swap
                    logger.info(f"         🎯 PRIORITY 5: Acceptable swap found!")
                    break

        # Add the best swap if found
        if best_swap:
            swap_candidates.append(best_swap)

            logger.info(f"         ✅ Aggressive geographic swap: {best_swap['user1_id']} ↔ {best_swap['user2_id']}")
            logger.info(f"            Proximity benefit: {best_swap['proximity_benefit']:.3f}km, User distance: {best_swap['user_distance']:.3f}km")
            logger.info(f"            Total score: {best_swap['improvement_score']:.3f}")
        else:
            logger.debug(f"         No suitable swaps found for routes {route1['driver_id']} and {route2['driver_id']}")
